(function() {
  ...
  exports = module.exports = BLEFirmata = (function(_super) {
    ...
    function BLEFirmata() {
      this.status = BLEFirmata.Status.CLOSE;
    ...
    BLEFirmata.prototype.connect = function(peripheral_name) {
      ...
      this.blendmicro = new BlendMicro(this.peripheral_name);
      console.log(this.blendmicro);
      this.blendmicro.once('open', (function(_this) {
        return function() {
          var cid;
          cid = setInterval(function() {
            debug('request REPORT_VERSION');
            return _this.write([BLEFirmata.REPORT_VERSION]);
          }, 500);
          _this.once('boardVersion', function(version) {
            clearInterval(cid);
            _this.status = BLEFirmata.Status.OPEN;
            return _this.emit('boardReady');
          });
          return _this.blendmicro.on('data', function(data) {
    ...
    BLEFirmata.prototype.isOpen = function() {
      return this.status === BLEFirmata.Status.OPEN;
    };

    BLEFirmata.prototype.close = function(callback) {
      this.status = BLEFirmata.Status.CLOSE;
      return this.blendmicro.close(callback);
    };
    ...
    BLEFirmata.prototype.write = function(bytes, callback) {
      return this.blendmicro.write(bytes, callback);
    };
